...
| TAT-108 | /tat report --type <bug\|improvement\|idea\|lesson> — real-time issue capture during TAT mode. Claude proactively calls this when it hits errors, spots patterns, or has insights. Logs to ~/.tinyaiteam/reports/ (global, not project .tat/) to avoid unstaged files in working repo (GL-19) and centralize cross-project reports. Sprint-end reads reports instead of relying on memory. Ref: cross-project lesson audit (Sprint 10), GL-19 | Sprint 10 |
| TAT-109 | Fix GPT review not captured — tat-code-review.sh should append GPT output to the existing review artifact (.tat/reviews/TASK-ID-review.md) instead of printing to stdout and losing it. One file per task: self-review section + GPT review section appended after. Ref: oneminuta Sprint 4 bug, GL-04 | Sprint 10 |
